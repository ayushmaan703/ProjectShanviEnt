...
const EpxencDetailsScreen = ({ navigation, route }) => {
    ...
    const onRefresh = async () => {
        setRefreshing(true);
        const responce = await dispatch(seeEmployeeExpences({ EmpId: data.EmpId, FromDate: data.FromDate, ToDate: data.ToDate, Paytype: PayType }));
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [dispatch, setPayType, PayType, navigation])
    ...
    const renderTask = ({ item }) => (
        PayType === 3 ?
            ...
            // onPress={() => navigation.navigate('EmployeeExpenceDetailsAdmin', { data: item, status: "Approved" })}
            >
            ...
            <Pressable
                ...
                onPress={() => navigation.navigate('EmployeeExpenceDetailsAdmin', { data: item, status: "Not Approved" })}
            >
                <Text style={[styles.cell, styles.expenseCell]}>{item.Expence}</Text>
                {/* <Text style={[styles.cell, styles.amountCell]}>{item.id}</Text> */}
                <Text style={[styles.cell, styles.amountCell]}>{item.BillAmount}</Text>
                <Text style={[styles.cell, styles.dateCell]}>{formattedDate(item.Date)}</Text>
                <Text style={[styles.cell, styles.remarkCell]}>
                    {item.Remark.length > 13 ? item.Remark.substring(0, 10) + '...' : item.Remark}
                </Text>
                {/* <View style={styles.statusCell}> */}
                {/* <FontAwesome6 name="thumbs-down" size={18} color="#EF4444" solid /> */}
                {/* <Text style={styles.statusText}>Not Approved</Text> */}
                {/* </View> */}
            </Pressable>
    ...
    return (
        <View style={styles.container}>
            ...
            <CustomNavBar title={"Expense Details"} color="rgba(123, 126, 209, 1)" />
            ...
            <View style={styles.content}>
                <View style={styles.titleContainer}>
                    ...
                    <Text style={styles.subtitleText}>
                        ...
                        {data.FromDate}  <Text style={styles.toText}>TO</Text>   {data.ToDate}
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyContainer}>
                            ...
                            <Text style={styles.emptyTitle}>No Expense</Text>
...
export default EpxencDetailsScreen;